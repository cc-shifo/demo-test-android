...
public class RangeSeekBar extends View {
    ...
    private void initProgressBitmap() {
        if (progressBitmap == null) {
            progressBitmap = Utils.drawableToBitmap(getContext(), progressWidth, progressHeight,
                    progressDrawableId);
        }
        if (progressDefaultBitmap == null) {
            progressDefaultBitmap = Utils.drawableToBitmap(getContext(), progressWidth,
                    progressHeight, progressDefaultDrawableId);
        }
    }
    ...
    private void initStepsBitmap() {
        ...
        if (stepsBitmaps.isEmpty()) {
            Bitmap bitmap = Utils.drawableToBitmap(getContext(), (int) stepsWidth,
                    (int) stepsHeight, stepsDrawableId);
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            ...
            tickMarkInRangeTextColor = t.getColor(
                    R.styleable.RangeSeekBar_rsb_tick_mark_in_range_text_color, progressColor);
    ...
    protected void onMeasureProgress(int w, int h) {
        ...
        if (gravity == Gravity.TOP) {
            ...
            if (leftSB.getIndicatorShowMode() != INDICATOR_ALWAYS_HIDE
                ...
                maxIndicatorHeight = Math.max(leftSB.getIndicatorRawHeight(),
                        rightSB.getIndicatorRawHeight());
            }
            float thumbHeight = Math.max(leftSB.getThumbScaleHeight(),
                    rightSB.getThumbScaleHeight());
            thumbHeight -= progressHeight / 2f;

            //default height is indicator + thumb exceeds the part of the progress bar
            //if tickMark height is greater than (indicator + thumb exceeds the part of the
            // progress)
            ...
            if (tickMarkTextArray != null && tickMarkLayoutGravity == Gravity.TOP) {
                progressTop = (int) Math.max(getTickMarkRawHeight(),
            ...
            if (tickMarkTextArray != null && tickMarkLayoutGravity == Gravity.BOTTOM) {
                ...
                progressBottom = (int) (viewHeight - Math.max(leftSB.getThumbScaleHeight(),
                        rightSB.getThumbScaleHeight()) / 2f
                        + progressHeight / 2f);
            }
        ...
        int maxThumbWidth = (int) Math.max(leftSB.getThumbScaleWidth(),
                rightSB.getThumbScaleWidth());
        ...
        progressDefaultDstRect.set(getProgressLeft(), getProgressTop(), getProgressRight(),
                getProgressBottom());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            heightSize = MeasureSpec.makeMeasureSpec(((ViewGroup) getParent()).getMeasuredHeight(),
                    MeasureSpec.AT_MOST);
        } else {
            int heightNeeded;
            if (gravity == Gravity.CENTER) {
                if (tickMarkTextArray != null && tickMarkLayoutGravity == Gravity.BOTTOM) {
                    ...
                    heightNeeded = (int) (2 * (getRawHeight() - Math.max(
                            leftSB.getThumbScaleHeight(), rightSB.getThumbScaleHeight()) / 2));
                }
            } else {
    ...
    protected int getTickMarkRawHeight() {
        if (tickMarkTextArray != null && tickMarkTextArray.length > 0) {
            return tickMarkTextMargin + Utils.measureText(String.valueOf(tickMarkTextArray[0]),
                    tickMarkTextSize).height() + 3;
        }
        return 0;
    }
    ...
    protected float getRawHeight() {
        ...
        if (seekBarMode == SEEKBAR_MODE_SINGLE) {
            ...
            if (tickMarkLayoutGravity == Gravity.BOTTOM && tickMarkTextArray != null) {
                float h = Math.max((leftSB.getThumbScaleHeight() - progressHeight) / 2,
        ...
        if (tickMarkTextArray != null) {
            ...
            for (int i = 0; i < tickMarkTextArray.length; i++) {
                ...
                if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
                    ...
                    if (Utils.compareFloat(num, states[0].value) != -1 && Utils.compareFloat(num,
                            states[1].value) != 1 && (seekBarMode == SEEKBAR_MODE_RANGE)) {
    ...
    protected void onDrawProgressBar(Canvas canvas, Paint paint) {
        ...
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            ...
            progressDstRect.left =
                    leftSB.left + leftSB.getThumbScaleWidth() / 2f + progressWidth * leftSB.currPercent;
            progressDstRect.right =
                    rightSB.left + rightSB.getThumbScaleWidth() / 2f + progressWidth * rightSB.currPercent;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (seekBarMode == SEEKBAR_MODE_RANGE) {
                    if (rightSB.currPercent >= 1 && leftSB.collide(getEventX(event),
                            getEventY(event))) {
                        currTouchSB = leftSB;
                        ...
                        float performClick =
                                (touchDownX - getProgressLeft()) * 1f / (progressWidth);
            ...
            case MotionEvent.ACTION_UP:
                if (verifyStepsMode() && stepsAutoBonding) {
                    ...
                    int stepSelected = new BigDecimal(percent / stepPercent).setScale(0,
    ...
    public void setProgress(float leftValue, float rightValue) {
        ...
        if (leftValue < minProgress) {
            throw new IllegalArgumentException(
                    "setProgress() min < (preset min - offsetValue) . #min:" + leftValue + " " +
                            "#preset min:" + rightValue);
        }
        if (rightValue > maxProgress) {
            throw new IllegalArgumentException(
                    "setProgress() max > (preset max - offsetValue) . #max:" + rightValue + " " +
                            "#preset max:" + rightValue);
        }
    ...
    public void setRange(float min, float max, float minInterval) {
        ...
        if (minInterval < 0) {
            throw new IllegalArgumentException(
                    "setRange() interval must be greater than zero ! #minInterval:" + minInterval);
        }
        if (minInterval >= max - min) {
            throw new IllegalArgumentException(
                    "setRange() interval must be less than (max - min) ! #minInterval:" + minInterval + " #max - min:" + (max - min));
        }
    ...
    /**
     * {@link #TICK_MARK_GRAVITY_LEFT} is number tick mark, it will locate the position according
     * to the value.
    ...
    public void setStepsDrawable(List<Integer> stepsDrawableIds) {
        ...
        if (!verifyStepsMode()) {
            throw new IllegalArgumentException(
                    "stepsWidth must > 0, stepsHeight must > 0,steps must > 0 First!!");
        }
        List<Bitmap> stepsBitmaps = new ArrayList<>();
        for (int i = 0; i < stepsDrawableIds.size(); i++) {
            stepsBitmaps.add(
                    Utils.drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight,
                            stepsDrawableIds.get(i)));
        }
