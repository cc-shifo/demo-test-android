...
public class CustomStatusView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mStatus == StatusEnum.Loading) {    //正在加载
            ...
            float tW = mPaint.measureText("59s");
            float tH = mPaint.descent() + mPaint.ascent();
            canvas.drawText("59s", progressRadius - tW / 2, progressRadius - tH / 2, mPaint);
            curAngle += 4;
            curAngle %= 360;
            canvas.rotate(curAngle, progressRadius, progressRadius);  //旋转的弧长为4
