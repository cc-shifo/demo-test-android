...
public class PoiAroundSearchActivity extends Activity implements OnClickListener,
    ...
    private final LatLonPoint lp = new LatLonPoint(39.993167, 116.473274);//
    ...
    private void init() {
        ...
        setup();
        mAMap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(lp.getLatitude(),
                lp.getLongitude()), 14));
    }
    ...
    protected void doSearchQuery() {
        currentPage = 0;
        query = new PoiSearch.Query(keyWord, "", city);// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi
        // 搜索区域（空字符串代表全国）
    ...
    private final int[] markers = {R.drawable.poi_marker_1,
