...
public class DriveRouteColorfulOverLay {
    ...
    private final List<Marker> throughPointMarkerList = new ArrayList<Marker>();
    ...
    private void colorWayUpdate(List<TMC> tmcSection) {
        ...
        for (int i = 0; i < mLatLngsOfPath.size() && j < tmcSection.size(); i++) {
            ...
            double distanceBetweenTwoPosition = AMapUtils.calculateLineDistance(startLatLng,
                    endLatLng);
            ...
            if (segmentTotalDistance > segmentTrafficStatus.getDistance() + 1) {
                double toSegDis =
                        distanceBetweenTwoPosition - (segmentTotalDistance - segmentTrafficStatus.getDistance());
