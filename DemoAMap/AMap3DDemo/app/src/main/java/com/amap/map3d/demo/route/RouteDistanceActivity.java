...
public class RouteDistanceActivity extends Activity implements DistanceSearch.OnDistanceSearchListener, AMap.OnMapLoadedListener {
    ...
    private final List<Text> mDistanceText = new ArrayList<Text>();


    // 北京站
    private final LatLonPoint start0 = new LatLonPoint(39.902896, 116.42792);
    // 北京南站
    private final LatLonPoint start1 = new LatLonPoint(39.865208, 116.378596);

    // 北京西站
    private final LatLonPoint start2 = new LatLonPoint(39.894914, 116.322062);

    // 北京北站
    private final LatLonPoint start3 = new LatLonPoint(39.945261, 116.352994);

    // 北京市政府
    private final LatLonPoint dest = new LatLonPoint(39.90455, 116.407555);
    ...
    private void setFromAndTo() {
        ...
        mDistanceText.add(aMap.addText(new TextOptions().position(getMidLatLng(latLngStart0,
                latLngEnd)).text("cal distance ...")));
        mDistanceText.add(aMap.addText(new TextOptions().position(getMidLatLng(latLngStart1,
                latLngEnd)).text("cal distance ...")));
        mDistanceText.add(aMap.addText(new TextOptions().position(getMidLatLng(latLngStart2,
                latLngEnd)).text("cal distance ...")));
        mDistanceText.add(aMap.addText(new TextOptions().position(getMidLatLng(latLngStart3,
                latLngEnd)).text("cal distance ...")));
        ...
        if (errorCode == AMapException.CODE_AMAP_SUCCESS) {
            try {
                ...
                for (DistanceItem item : distanceItems) {
                    ...
                    stringBuffer.append("\n");
                    Log.i("amap", "onDistanceSearched " + index + " : " + stringBuffer);
