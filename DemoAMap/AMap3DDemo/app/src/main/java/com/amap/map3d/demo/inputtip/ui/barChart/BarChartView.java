...
public class BarChartView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        maxbarChartHeight =
                height - nameTextSize - lineWidth - distanceFormNameToLine - valueTobarChartDistance - valueTextSize - chartPaddingTop;
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                for (int i = 0; i < dataList.size(); i++) {
                    ...
                    Log.d("text ", "i:" + i + "getPositionX:" + groupData.getPositionX() + " " +
                            "barChartbarChartWidth:" + barChartWidth);
                ...
                Log.d("text",
                        "clickedPosition:" + clickedPosition + " scroller.getCurrX():" + scroller.getCurrX());
    ...
    private void fling(int velocityX) {
        if (Math.abs(velocityX) > minimumVelocity) {
            ...
            scroller.fling(getScrollX(), getScrollY(), -velocityX, 0, 0,
                    barChartTotalWidth + paddingStart - width, 0, 0);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //canvas.drawLine(coordinateAxisWidth / 2 + scrollX, 0, coordinateAxisWidth / 2 +
        // scrollX, axisBottom, coordinateAxisPaint);
        ...
        if (dataList != null && dataList.size() > 0) {
            ...
            for (BarChartData date : dataList) {
                ...
                int barChartHeight = maxValue == 0 ? 0 :
                        (int) (date.getValue() / maxValue * maxbarChartHeight);


                barChartPaintRect.top =
                        height - barChartHeight - lineWidth - distanceFormNameToLine - nameTextSize;
                ...
                if (barChartShaderColor != null && barChartShaderColor.length > 0) {
                    shader = getbarChartShader(barChartPaintRect.left,
                            chartPaddingTop + valueTobarChartDistance + valueTextSize,
                ...
                String barChartHeightValue =
                        StringUtil.NumericScaleByFloor(String.valueOf(date.getValue()), 0) + date.getUnit();

                if (pos == clickedPosition) {
                    float valueTextX =
                            xOffset + (barChartWidth - clickedValuePaint.measureText(barChartHeightValue)) / 2;
                    float valueTextY =
                            barChartPaintRect.top - distanceFormNameToLine + (valueFontMetrics.bottom) / 2;
                    canvas.drawText(barChartHeightValue, valueTextX, valueTextY, clickedValuePaint);
                } else {
                    float valueTextX =
                            xOffset + (barChartWidth - valuePaint.measureText(barChartHeightValue)) / 2;
                    float valueTextY =
                            barChartPaintRect.top - distanceFormNameToLine + (valueFontMetrics.bottom) / 2;
                ...
                if (/*pos == clickedPosition*/false) {
                    ...
                    float nameTextX =
                            xOffset - groupWidth - barChartInterval + (groupWidth - nameTextWidth) / 2;
