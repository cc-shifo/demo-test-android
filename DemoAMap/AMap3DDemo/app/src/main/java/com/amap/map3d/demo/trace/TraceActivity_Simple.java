...
public class TraceActivity_Simple extends Activity implements TraceStatusListener,
    ...
    @Override
    public void onTraceStatus(List<TraceLocation> locations, List<LatLng> rectifications,
                              String errorInfo) {
        try {
            if (!LBSTraceClient.TRACE_SUCCESS.equals(errorInfo)) {
                Log.e("amap", " source count->" + (locations == null ? "0" : locations.size()) +
                        "   result count->" + (rectifications == null ? "0" :
                        rectifications.size()));
