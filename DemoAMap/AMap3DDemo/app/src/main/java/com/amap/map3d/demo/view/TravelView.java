...
public class TravelView extends ConstraintLayout {
    ...
    //提前十分钟
    private static final long diffTime = 60 * 5;
    ...
    public void init(final DriveRoutePlanResult result, final IndexListener indexListener,
                     long arriveTime) {
    ...
    private void setPathInfo(final DriveRoutePlanResult result, int index, boolean setDistance) {
        if (index >= 0 && result.getTimeInfos().size() > index) {
            ...
            if (setDistance) {
                ...
                distance.setText(element.getDuration() + "分");
            }
        } else {
