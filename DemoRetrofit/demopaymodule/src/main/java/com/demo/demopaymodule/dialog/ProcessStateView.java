...
import com.demo.demopaymodule.utils.DisplayUtils;
...
public class ProcessStateView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mState == State.STATE_PROCESSING) {
            ...
            if (mStateImage != null) {
                final Rect bounds = mStateImage.getBounds();
                if (mRectF.width() != bounds.right ||
                        mRectF.height() != bounds.bottom) {
                    mStateImage.setBounds(0, 0, (int) mRectF.width(), (int) mRectF.height());
                }
                canvas.translate(mRectF.left, mRectF.top);
                mStateImage.draw(canvas);
            }
    ...
    private void initFailureAnimation() {
        ...
        mFailureAnimRight.setDuration(50);
        Path path = new Path();
        path.moveTo(0f, 0f);
