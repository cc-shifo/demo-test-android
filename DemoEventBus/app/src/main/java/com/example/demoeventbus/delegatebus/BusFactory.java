...
class BusFactory {
    ...
    @NonNull
    public <T> ILiveDataWrapper<T> create(EventWrapper eventWrapper) {
    ...
    static final class EventGroupHolder {
        final Map<String, ILiveDataWrapper<?>> eventBus = new HashMap<>();

        EventGroupHolder(EventWrapper eventWrapper) {
            if (!eventBus.containsKey(eventWrapper.event)) {
                eventBus.put(eventWrapper.event, new ImplLiveDataWrapper(eventWrapper));
            }
        }

        @SuppressWarnings("unchecked")
        <T> ILiveDataWrapper<T> getBus(EventWrapper eventWrapper) {
            ILiveDataWrapper<T> bus;
            //  一个分组不会有相同的事件名，即使类型不一样也不行，定义事件时就会报错
            // 如果用户不使用事件定义方式，很难保证事件一致
            if (eventBus.containsKey(eventWrapper.event)) {
                bus = (ILiveDataWrapper<T>) eventBus.get(eventWrapper.event);
            } else {
                synchronized (eventBus) {
                    if (eventBus.containsKey(eventWrapper.event)) {
                        bus = (ILiveDataWrapper<T>) eventBus.get(eventWrapper.event);
                    } else {
                        bus = new ImplLiveDataWrapper<>(eventWrapper);
