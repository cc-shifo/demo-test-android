 ...
 * 3、动态刻度值。
 */
public class DemoVRightRulerView extends View {
    /**
    ...
    public DemoVRightRulerView(Context context) {
        this(context, null);
    }
    ...
    public DemoVRightRulerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public DemoVRightRulerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr,
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.DemoVRightRulerView);
        mRulerSize = array.getDimensionPixelSize(R.styleable.DemoVRightRulerView_vr_rulerSize, 4);
        mSegmentSize =
                array.getDimensionPixelSize(R.styleable.DemoVRightRulerView_vr_segmentSize, 5);
        mScaleSize = array.getFloat(R.styleable.DemoVRightRulerView_vr_scaleSize, 5f);
        mSLLengthMax =
                array.getDimensionPixelSize(R.styleable.DemoVRightRulerView_vr_scaleLineMax, 24);
        mSLLengthMin =
                array.getDimensionPixelSize(R.styleable.DemoVRightRulerView_vr_scaleLineMin, 12);
        mSLStrokeMax =
                array.getDimensionPixelSize(R.styleable.DemoVRightRulerView_vr_scaleLineStrokeMax,
                        8);
        mSLStrokeMin =
                array.getDimensionPixelSize(R.styleable.DemoVRightRulerView_vr_scaleLineStrokeMax,
                        4);
        mScaleTextSize =
                array.getDimensionPixelSize(R.styleable.DemoVRightRulerView_vr_scaleTextSize, 32);
        mScaleTextColor =
                array.getColor(R.styleable.DemoVRightRulerView_vr_scaleTextColor, Color.GREEN);

        mHintTextBoldSize = array.getDimensionPixelSize(R.styleable
                .DemoVRightRulerView_vr_hintTextBoldSize, 64);
        mHintTextMiddleSize = array.getDimensionPixelSize(R.styleable
                .DemoVRightRulerView_vr_hintTextMiddleSize, 32);
        mHintTextSmallSize = array.getDimensionPixelSize(R.styleable
                .DemoVRightRulerView_vr_hintTextSmallSize, 24);
        mHintLabelTextColor = array.getColor(R.styleable.DemoVRightRulerView_vr_hintLabelTextColor,
                Color.WHITE);
        mHintTextColor =
                array.getColor(R.styleable.DemoVRightRulerView_vr_hintTextColor, Color.GREEN);
        array.recycle();
        init();
    }
     ...
     * 提示信息
     */
    private static final String HINT_TEXT = "000.0";
    private static final String HINT_LABEL = "ALT";
    private static final String HINT_UNIT = "m";
    /**
     * 提示信息内部填充边距，单位像素。
     */
    ...
    private void initLayout() {
        ...
        mHintPaint.setTextSize(mHintTextSmallSize);
        metrics = mHintPaint.getFontMetrics();
        tW = mHintPaint.measureText(HINT_UNIT);
        mHintUnitX = mHintRight - HINT_PADDING - tW;
    ...
    private void drawRuleLine(Canvas canvas) {
        ...
        mScalePaint.setStrokeWidth(mSLStrokeMax);
        canvas.drawLine(mRulerX, mRulerY, mRulerX,
                mRulerY + mRulerHeight, mScalePaint);

        // 上下两端的边界线段，线包含在区域内
        canvas.drawLine(0, mRulerY + mSLStrokeMax / 2f, mSLLengthMax, mRulerY + mSLStrokeMax / 2f,
                mScalePaint);
        float bottom = mRulerY + mRulerHeight - mSLStrokeMax / 2f;
        canvas.drawLine(0, bottom, mSLLengthMax, bottom, mScalePaint);
    }
    ...
    private void drawScaleLine(Canvas canvas) {
        ...
        //         }
        //         drawMaxScaleLine(canvas, y);
        //         drawMaxScaleText(canvas, y, (i + bottomVal) / mSegmentSize);
        //     } else {// 短刻度线
        //         drawMinScaleLine(canvas, y);
        //     }
        //     index++;
     ...
     * 绘制长刻度线
     */
    private void drawMaxScaleLine(Canvas canvas, float y) {
        mScalePaint.setStrokeWidth(mSLStrokeMax);
        // 超出线段不绘制
        if (y >= mRulerY + mSLStrokeMax / 2f && y <= mRulerY + mRulerHeight - mSLStrokeMax / 2f) {
            canvas.drawLine(0, y, mSLLengthMax, y, mScalePaint);
        }
    }
     ...
     */
    private void drawMaxScaleText(Canvas canvas, float y, int value) {
        // 指针线位置的刻度值不画，跳过
        if (y < mHintTop - mScaleTextHeight || y > mHintBottom + mScaleTextHeight) {
            ...
            canvas.drawText(text, mSLLengthMax + MARGIN_CURSOR, y - tH / 2, mScaleTextPaint);
        }
    }
     ...
     * @param y      刻度线垂直方向中心线坐标。
     */
    private void drawMinScaleLine(Canvas canvas, float y) {
        mScalePaint.setStrokeWidth(mSLStrokeMin);
        y -= mSLStrokeMin / 2f;
        // 超出线段不绘制
        if (y >= mRulerY + mSLStrokeMax / 2f && y <= mRulerY + mRulerHeight - mSLStrokeMax / 2f) {
            canvas.drawLine(0, y, mSLLengthMin, y, mScalePaint);
        }
    }
