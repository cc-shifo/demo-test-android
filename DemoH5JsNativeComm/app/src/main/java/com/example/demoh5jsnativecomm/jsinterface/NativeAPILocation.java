...
public class NativeAPILocation {
    ...
    public String getLocation() {
        synchronized (NativeAPILocation.this) {
            if (mIsGetting) {
                return mLocationCache != null ? mLocationCache : new Gson().toJson(
                        new NativeAPILocationResult(NativeAPIResultCode.LOCATION_NOT_PREPARED,
                                new NativeAPILocationResult.Location(Double.NaN, Double.NaN,
                                        Double.NaN)));
            }
            mIsGetting = true;
            if (EasyPermissions.hasPermissions(mContext, LocPermissionString)) {
                ...
                mLocationCache = new Gson().toJson(new NativeAPILocationResult(
                        NativeAPIResultCode.REQUEST_PERMISSION, new NativeAPILocationResult
