...
import com.maptiler.simplemap.databinding.ActivityTestTianDiMapBinding;
...
public class TestTianDiMapWMSActivity extends AppCompatActivity {
    private static final String TAG = "Test01Activity";
    private static final String TAGABC = "TAGABC";
    private ActivityTestTianDiMapBinding mBinding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = ActivityTestTianDiMapBinding.inflate(getLayoutInflater());
        ...
        //     mapboxMap.clear();
        //     // 设置天地图的样式
        //     // mapboxMap.setStyle(new Style.Builder().fromUri("https://api.maptiler.com/maps/streets/style.json"), new Style.OnStyleLoaded() {
    ...
    private void addTiandituLayerDeepSeek01(@NonNull Style style) {
        ...
        TileSet tileSet = new TileSet("tianditu-vec", "http://t0.tianditu.gov.cn/vec_w/wmts?service=wmts"
                + "&request=GetTile"
                + "&version=1.0.0&layer=vec&style=default"
                + "&tilematrixset=w&format=tiles&tilematrix={z}&tilerow={y}&tilecol={x}&tk="
                + "660418457de5e147896a7c4d1dd5f771");
     ...
     * 影像图
     */
    private void addTianDiMapCopilotWMS(@NonNull Style style) {
        String tiandituUrl = "https://t0.tianditu.gov.cn/img_w/wmts?tk=660418457de5e147896a7c4d1dd5f771"
        ...
        // String tiandituLabelUrl = "https://t0.tianditu.gov.cn/cia_w/wmts?tk=660418457de5e147896a7c4d1dd5f771"
        //         + "&service=wmts&request=GetTile&version=1.0.0&layer=cia&style=default&tilematrixset=w"
        //         + "&format=tiles&tilematrix={z}&tilerow={y}&tilecol={x}";
        // TileSet labelTileSet = new TileSet("tileset", tiandituLabelUrl);
        // labelTileSet.setScheme("xyz");
        // RasterSource labelRasterSource = new RasterSource("tianditu-label-source", labelTileSet, 256);
        // style.addSource(labelRasterSource);
        //
        // RasterLayer labelRasterLayer = new RasterLayer("tianditu-label-layer", "tianditu-label-source");
        // style.addLayer(labelRasterLayer);


        RasterSource rasterSourceMarker = new RasterSource("tianditu-cia_w", new TileSet(
              "tianditu-tile-cia_w", "http://t0.tianditu.gov.cn/cia_w/wmts?service=wmts&request=GetTile"
                + "&version=1.0.0&layer=cia&style=default&tilematrixset=w&format=tiles"
                + "&tilematrix={z}&tilerow={y}&tilecol={x}&tk="
                + "660418457de5e147896a7c4d1dd5f771"
        ), 256);
        style.addSource(rasterSourceMarker);

        RasterLayer rasterLayerMarker = new RasterLayer("tianditu-layer-cia_w", "tianditu-cia_w");
        // style.addLayer(rasterLayerMarker);
        style.addLayerAbove(rasterLayerMarker, "tianditu-layer"); // 将标注图层放在矢量图层之上
     ...
     * 矢量图
     */
    private void addTianDiMapDeepSpeek02(@NonNull Style style) {
        // 设置天地图的样式
        // 在这里可以添加其他图层或标记
        ...
        style.addSource(new RasterSource("tianditu-cva-source",
                new TileSet("tianditu-cva", "http://t0.tianditu.gov.cn/cva_w/wmts?service=wmts&request=GetTile"
                        + "&version=1.0.0&layer=cva&style=default"
                        + "&tilematrixset=w&format=tiles&tilematrix={z}&tilerow={y}&tilecol={x}&tk="
                        + "660418457de5e147896a7c4d1dd5f771"),
                256));
    ...
    private void addCustomInfoWindowAdapter(MapboxMap map) {
        MapboxMap.InfoWindowAdapter adapter = marker -> {
            TextView tv = new TextView(TestTianDiMapWMSActivity.this);
            tv.setWidth(200);
            tv.setHeight(100);
            tv.setText("info");
    ...
    private void btnAddOneMarker() {
        mBinding.btnAddMarker.setOnClickListener(v -> {
            Icon icon = IconFactory.getInstance(TestTianDiMapWMSActivity.this)
                    .fromResource(R.drawable.test_dot);
                ...
                mDisposables.add(Completable.create(emitter -> {
                            mMap.snapshot(snapshot -> { // 测试发现截屏操作必须在主线程中进行
                                ...
                                if (mScreenShotView == null) {
                                    mScreenShotView = new ImageView(TestTianDiMapWMSActivity.this);
